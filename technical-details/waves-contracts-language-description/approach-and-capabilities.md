...
In order to setup an Smart Account, The account needs to issue [`SetScriptTransaction`](https://ebceu4.github.io/neel-transactions/interfaces/setscripttransaction.html) which contains the predicate. Upon success, every outgoing transaction will be validated not by the default mechanism of signature validation, but according to the predicate logic.  
`AccountScript` can be changed or cleared if the script installed allows the new [`SetScriptTransaction`](https://ebceu4.github.io/neel-transactions/interfaces/setscripttransaction.html) to process.  
...
**Note.** [`SetScriptTransaction`](https://ebceu4.github.io/neel-transactions/interfaces/setscripttransaction.html) sets the script which verifies all outgoing transactions. The set script can be changed by another [`SetScriptTransaction`](https://ebceu4.github.io/neel-transactions/interfaces/setscripttransaction.html) call unless it’s prohibited by a previous set script.
...
* The fixed cost for each scripted unit is equal to **400000 **_**wavelets**_ \(Neel coins, 100000000 wavelets = 1 Wave\).

**Examples:**

* If transaction's usual cost is 0.001 NEEL, in case of smart scripting exactly the same transaction will be cost 0.001 + 0.004 NEEL. 
...
Neel also added trader’s script check in Matcher. When it receives an order from a smart account, it executes the script for the order.
...
#### 2. Buy back custom asset on specified price in NEEL:
