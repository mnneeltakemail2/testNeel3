...
There are currently 13 different types of transactions in the Neel Blockchain. the following table shows the minimal required fees in non-scripted case \(accounts don’t have scripts, assets don’t have scripts\) and for each time the script is called, total transaction’s fee increases by 0.004 Neel.

**Note. **If a scripted account transfers a smart asset, then the fee is increased twice \(the fee increases _**+0.004**_ every time the transaction is validated by account’s script or asset’s script\), you can check here the [_**transactions examples**_](../development-and-api/neel-node-rest-api/example-transactions.md).

| Transaction | Minimal Transaction Fee in NEEL |
| :--- | :--- |
| Issue | 1 |
| Transfer | 0.001 |
| Reissue | 1 |
...
* plain transfer fee is **0.001** NEEL, if user makes account scripted or smart asset transfer, the fee should be 0.005.neel, but if user will transfer smart assets from scripted account the final fee is **0.009** NEEL.
* Exchange transaction fee is **0.003** NEEL, if matcher also can be scripted, this makes fee as **0.007** NEEL .
