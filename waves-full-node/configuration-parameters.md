...
```java -jar neel-all-0.13.3.jar neel.conf```

Typically this file should contain you node's unique characteristics (ip, name, keys, etc...) and network-specific parameters similar to neel-mainnet or neel-testnet configs from previous sections (files shipped with DEB packages).
...
**Note:** If you want to change neel directory in Ubuntu packages you should change it using `-J-Dneel.directory=path` in `/etc/neel/application.ini` and `/lib/systemd/system/neel.service`. You can override any JVM start parameter in `neel.service`, it has priority.
