...
3. [Neel DEX](#section-eebc687574be0316202adddb271c7f74)
...
2. If there is a counter-order that matches with the submitted order, then the order execution is performed. That means the counter-order is removed from order book and the matcher creates exchange transaction, signs it by matcher's private key and is sent to the Neel network for including in the blockchain.
...
The fix full transaction fee now is equal to **0.003 Neel for one order**, whether buying or selling, regardless of the amount of the future deal. The exchange transaction contains two separate fields for Matcher's fee, which goes from buyer's order and seller's order. An order can be fully executed by some transaction, in this case, all matcher fee from it is included in that transaction.
...
There are 3 different orders \(Figure 2\): two buy orders and one sell. For each full order, a user has to pay exactly **0.003 neel** of a fee, and this fee will be written off as the order is executed. In our example:

* the Order1 is fully matched with a 70% part of Order3 by Transaction1 and matcher's fee for this transaction is equal to 0.003 + 0.0021 - 0.003 = 0.0021 neel since Matcher pay to miners transaction fee which is also equal to 0.003 Neel.
* The 50% of Order2 matches with 30% part of Order3 by Transaction2 and matcher's fee for this transaction is equal to 0.0009 + 0.0015 - 0.003 = -0.0006 neel.

Thus, the fee that the matcher gets from users for these transactions is**0.0021 - 0.0006  = 0.0015 neel**. And the fee that the matcher pays to miners is**0.006neel**.
...
| :--- | :--- | :--- | :--- |
| last month | 681.42336675 neel | 569.721 neel | 16.39% |
| all time | 3476.01418346 neel | 2824.771 neel | 18.74% |
...
* [NEEL](../development-and-api/neel-node-rest-api/address.md#get-addressesbalanceaddress);
* [assets](../development-and-api/neel-node-rest-api/address.md#get-assetsbalanceaddress).

## spendings of asset in transactions in utx pool

The sum of all spending by the asset of unconfirmed transactions.

For example, if you are transferring `NEEL` and sending a [data transaction](../technical-details/data-transaction.md)  
and these transactions haven't yet forged, `spendings_of_NEEL_in_transactions_in_utx_pool` will be `amount_of_transferred_neel + transfer_fee + data_transaction_fee`.
...
Note, for `NEEL` order's fees are included in reserved balance.

As you know, all orders in `DEX` requires `NEEL` as the fee. Because of this, there is an exception for the rule we spoke above.  
If you buy `NEEL` by other asset, reserved balance in neel in this order will be: `max(fee - amount_of_received_neel_in_this_order, 0)`.  
So, you can buy `NEEL` for `BTC` even if you have no `NEEL`.
...
* Users can install their own matcher by installing the Neel software and enabling the matching functionality.
